...
### 使 syslogd 能够适应除 GMT 之外的时区

一些 `syslog()` 实现（例如 musl）总是以 UTC 发送时间戳。以下代码为 `syslogd` 添加了一个新选项 `-Z`，以假定传入的时间戳始终为 UTC，并在记录之前将它们调整为本地时区（syslogd 的时区）。
...
_from  [sysklogd：添加 -Z 选项以调整消息时区](http://lists.busybox.net/pipermail/busybox/2017-May/085437.html)_
